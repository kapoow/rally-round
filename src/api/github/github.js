...
const runGitHubOperations = async () => {
  ...
  if (!process.env.CLUB || !process.env.GITHUB_ACCESS_TOKEN) {
    console.error(
      "Error: Environment variables CLUB or GITHUB_ACCESS_TOKEN are not set. Exiting..."
    );
    process.exit(1);
  }
  ...
  if (!process.env.GITHUB_ORG) {
    console.error(
      "Error: Environment variable GITHUB_ORG is not set. Exiting..."
    );
    process.exit(1);
  }

  const owner = process.env.GITHUB_ORG; // GitHub organization/username
