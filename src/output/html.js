...
const writeHomeHTML = links => {
    ...
    if (division.events && division.events.length > 0) {
      division.events.forEach(event => {
        if (event.status === eventStatuses.active) {
          activeEvents.push({
            name: event.name,
            location: event.location?.countryName || "",
  ...
  const rules = {
    ...
    powerStagePoints:
      league.divisions[Object.keys(league.divisions)[0]]?.points?.powerStage ||
      [],
    overallPointsTop5:
      league.divisions[
        Object.keys(league.divisions)[0]
      ]?.points?.overall?.slice(0, 5) || []
  };
